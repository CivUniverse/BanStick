...
public class IpCheckImportWorker extends ImportWorker {
	...
	private boolean provisionDatabase(ConfigurationSection config) {
		...
		try {
			this.internalDatabase = ManagedDatasource.construct(BanStick.getPlugin(), new DatabaseCredentials(
					username,
					password,
					host,
					port,
			...
			assert this.internalDatabase != null;
			this.internalDatabase.getConnection().close();
		}
